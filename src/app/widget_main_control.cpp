...
void WidgetMainControl::onApplicationItemSelectionChanged()
{
    ...
    if (spanAppItem.size() == 1) {
        ...
        if (appItem.isDocument()) {
            ...
            std::vector<TGraphicsObjectExt>& vecGfxObjectExt = guiDoc->getSelGfxObjectExts();
            vecGfxObjectExt.clear();
            //std::vector<GraphicsObjectPtr> vecGfxObject;
            std::vector<GraphicsObjectPtr>& vecGfxObject = guiDoc->getSelGfxObjects();
            vecGfxObject.clear();
            guiDoc->foreachGraphicsObject(docTreeNode.id(), [&](GraphicsObjectPtr gfxObject, TreeNodeId tnId) {
                vecGfxObject.push_back(std::move(gfxObject));
                TGraphicsObjectExt gfxObjectExt;
                gfxObjectExt.obj = std::move(gfxObject);
                gfxObjectExt.lbl = guiDoc->document()->modelTree().nodeData(tnId);
                gfxObjectExt.tnId = tnId;
                gfxObjectExt.name = uiModelTree->getItemText(guiDoc->document(), tnId);
                vecGfxObjectExt.push_back(gfxObjectExt);
            });
...
WidgetGuiDocument* WidgetMainControl::latestWidgetGuiDocument()
{
    WidgetGuiDocument* tmpWidgetGuiDocument = widgetGuiDocument(m_newWidgetGuiDocIndex);
    return tmpWidgetGuiDocument;
}

int WidgetMainControl::latestWidgetGuiDocumentIndex()
{
    return m_newWidgetGuiDocIndex;
}
...
void WidgetMainControl::onGuiDocumentAdded(GuiDocument* guiDoc)
{
    ...
    widgetCtrl->signalMouseMoved.connectSlot([=](int xPos, int yPos) {
        ...
        gp_Pnt pos = widgetCtrl->getTransform();
        m_ui->label_ValuePosX->setText(QString::number(pos.X(), 'f', 3));
        m_ui->label_ValuePosY->setText(QString::number(pos.Y(), 'f', 3));
        m_ui->label_ValuePosZ->setText(QString::number(pos.Z(), 'f', 3));
    });
    ...
    m_newWidgetGuiDocIndex = newDocIndex;
}
