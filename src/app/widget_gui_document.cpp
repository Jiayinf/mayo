...
WidgetGuiDocument::WidgetGuiDocument(GuiDocument* guiDoc, QWidget* parent)
    : QWidget(parent),
      ...
      m_controller(new WidgetOccViewController(m_qtOccView, guiDoc->graphicsScene()->aisContextPtr()))
{
    {
    ...
    m_btnMove = this->createViewBtn(widgetBtnsContents, Theme::Icon::Measure, tr("Move"));
    ...
    QObject::connect(m_btnMove, &ButtonFlat::checked, this, &WidgetGuiDocument::toggleWidgetMove);
...
void WidgetGuiDocument::toggleWidgetMove(bool on)
{
    if (!m_controller->getManipulatorReady())
    {
        std::vector<GraphicsObjectPtr>& selAisTrihedrons = m_guiDoc->getSelAisTrihedrons();
        if (0 < selAisTrihedrons.size())
        {
            float mat[12] = { 0 };
            gp_Trsf trsf = selAisTrihedrons.at(0)->Transformation();
            occTrsfToMat(trsf, mat);
            m_controller->startManipulator(selAisTrihedrons.at(0), mat);

            emit sig_requestOccTransRot(true, -1, 1);
        }
        else
        {
            std::vector<GraphicsObjectPtr>& gfxObjects = m_guiDoc->getSelGfxObjects();
            if (0 == gfxObjects.size())
            {
                return;
            }

            m_gfxObjExts = m_guiDoc->getSelGfxObjectExts();

            m_controller->startManipulator(gfxObjects);

            if (1 == gfxObjects.size())
            {
                emit sig_requestOccTransRot(true, -1, 4);
            }
        }
        m_controller->setManipulatorReady(true);
    }
    else
    {
        m_controller->stopManipulator();

        m_controller->setManipulatorReady(false);

        ///*DlgOccTransRot* occTransRotDlg = DlgOccTransRot::instance(this);
        //occTransRotDlg->close();*/

        emit sig_requestOccTransRot(false, 0, 0);
    }
}
