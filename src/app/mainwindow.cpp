      ...
      m_ui(new Ui_MainWindow)
{
    ...
    //connect(this->widgetPageDocuments(), &WidgetGuiDocument::sig_requestOccTransRot, this, &MainWindow::on_requestOccTransRot);
}
...
void MainWindow::on_requestOccTransRot(bool show, int id, int type)
{
    Mayo::WidgetGuiDocument* tmpGuiDoc = this->widgetPageDocuments()->latestWidgetGuiDocument();
    if (nullptr == tmpGuiDoc)
        return;

    /*DlgOccTransRot* occTransRotDlg = DlgOccTransRot::instance(this);*/
    //connect(occTransRotDlg, &DlgOccTransRot::sig_closeOccTransRot, this, &CMainWindow::on_closeOccTransRot);
    if (show) {
        std::vector<GraphicsObjectPtr>& selAisTrihedrons = tmpGuiDoc->guiDocument()->getSelAisTrihedrons();
        //if ((type == 3) || (0 == type))
        //{
        //    occTransRotDlg->init(id, tmpGuiDoc->guiDocument(), tmpGuiDoc->controller(), type);
        //}
        //else {
        //    if (0 < selAisTrihedrons.size()) {
        //        occTransRotDlg->init(selAisTrihedrons.at(0), tmpGuiDoc->guiDocument(), tmpGuiDoc->controller(), type);
        //    }
        //    else {
        //        std::vector<GraphicsObjectPtr>& gfxObjects = tmpGuiDoc->guiDocument()->getSelGfxObjects();
        //        occTransRotDlg->init(gfxObjects.at(0), tmpGuiDoc->guiDocument(), tmpGuiDoc->controller(), type);
        //    }
        //}

        //occTransRotDlg->show();
        //_rDockWidget->setWidget(/*(QWidget*)*/occTransRotDlg);
        //_rDockWidget->show();

        //_rdockTitle->show();

        //addDockWidget(Qt::RightDockWidgetArea, (QDockWidget*)_rDockWidget);
    }
    else {
        //on_closeOccTransRot();
    }
