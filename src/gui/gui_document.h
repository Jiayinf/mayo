...
public:
    ...
    std::vector<GraphicsObjectPtr>& getSelGfxObjects();

    std::vector<Handle(AIS_InteractiveObject)>& getSelAisTrihedrons();

    std::vector<TGraphicsObjectExt>& getSelGfxObjectExts();
    ...
    void foreachGraphicsObject(TreeNodeId nodeId, const std::function<void(GraphicsObjectPtr, TreeNodeId)>& fn) const;
...
private:
    ...
    std::vector<GraphicsObjectPtr> m_selGfxObjects;



    void clearSelAisTrihedrons();

    std::vector<Handle(AIS_InteractiveObject)> m_selAisTrihedrons;

    std::vector<TGraphicsObjectExt> m_selGfxObjectExts;
};
