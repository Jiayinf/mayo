...
std::vector<GraphicsObjectPtr>& GuiDocument::getSelGfxObjects()
{
    return m_selGfxObjects;
}

std::vector<Handle(AIS_InteractiveObject)>& GuiDocument::getSelAisTrihedrons()
{
    return m_selAisTrihedrons;
}

std::vector<TGraphicsObjectExt>& GuiDocument::getSelGfxObjectExts()
{
    return m_selGfxObjectExts;
}
...
void GuiDocument::setDevicePixelRatio(double ratio)
{
    ...
    case ViewTrihedronMode::AisViewCube: {
        ...
        if (viewCube) {
            viewCube->SetSize(35 * m_devicePixelRatio, true/*adaptOtherParams*/);
            viewCube->SetFontHeight(10 * m_devicePixelRatio);
            const int xyOffset = std::lround(60 * m_devicePixelRatio);
...
void GuiDocument::foreachGraphicsObject(
        TreeNodeId nodeId, const std::function<void (GraphicsObjectPtr, TreeNodeId)>& fn) const
{
    if (!fn)
        return;
